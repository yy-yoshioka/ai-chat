...
const EncryptionPage = () => {
  ...
  const [selectedTab, setSelectedTab] = useState<'overview' | 'keys' | 'compliance' | 'audit'>(
    'overview'
  );
  ...
  const loadEncryptionData = useCallback(async () => {
    try {
      const [statusResponse, keysResponse, policiesResponse] = await Promise.all([
        fetch(`/api/organizations/${id}/encryption/status`),
  ...
  }, [id, loadEncryptionData]);

  const createEncryptionKey = async (keyData: unknown) => {
    try {
  ...
  const rotateEncryptionKey = async (keyId: string, config: Record<string, unknown>) => {
    try {
      ...
      <div className="space-y-6">
        ...
        <div className="border-b border-gray-200">
          <nav className="-mb-px flex space-x-8">
            {[
              { key: 'overview', label: '📊 概要', desc: '暗号化状態と統計' },
              ...
              <button
                key={key}
                onClick={() => setSelectedTab(key as 'overview' | 'keys' | 'compliance' | 'audit')}
                ...
                <div key={key.id} className="bg-white rounded-lg shadow-sm p-6">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button
                        onClick={() => rotateEncryptionKey(key.id, {})}
                      ...
                      <div className="space-y-3">
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="font-medium">
                              {key.usage.encrypt.toLocaleString()}
                            </span>
                          </div>
                        ...
                        <div>
                          <div className="flex justify-between text-sm mb-1">
                            ...
                            <span className="font-medium">
                              {key.usage.decrypt.toLocaleString()}
                            </span>
                          </div>
                          <div className="w-full bg-gray-200 rounded-full h-2">
                            <div
