...
router.get(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const ipStatus = await chatRateLimiter.getStatus(
        `chat_rl_ip:${ip}`,
        {
          windowMs: 60 * 1000,
          maxRequests: 100,
      ...
      res.json({
        ip: {
          ...
          resetTime: ipStatus?.resetTime ? new Date(ipStatus.resetTime).toISOString() : null,
        },
        organization: organizationId ? {
          ...
          resetTime: orgStatus?.resetTime ? new Date(orgStatus.resetTime).toISOString() : null,
        } : null,
      });
    } catch (error) {
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (type === 'ip') {
        ...
        return res.status(400).json({ error: 'Invalid type. Must be "ip" or "organization"' });
      }
